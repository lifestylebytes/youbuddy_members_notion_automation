...
async function getRelationPropertyName(dataSourceId) {
  ...
  const relationProperty = Object.values(dataSource.properties || {}).find(
    (property) =>
      ...
      config.normalizedTeamDataSourceIds.includes(config.normalizeId(property.relation?.data_source_id))
  );
...
async function main() {
  ...
  for (const member of members) {
    ...
    if (!relationPropertyName) {
      console.log(`${member.name}: no team relation property in original 4기, skipping`);
      continue;
    }
